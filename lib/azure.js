  ...
  return {
    ...
    listContainers: () => {
      return new Promise((resolve, reject) => {
        ...
        function getList(continuationToken) {
          blobClient.listContainersSegmented(
            continuationToken,
            (err, result) => {
              if (err) return reject(err)
              if (result.continuationToken) {
                return getList(result.continuationToken)
              }

              result.entries.forEach(x => allResults.push(x))
    ...
    deleteContainer: containerName => {
      return new Promise((resolve, reject) => {
        blobClient.deleteContainerIfExists(containerName, err => {
          if (err) return reject(err)
          resolve()
        })
      })
